...
/**
 * Nigeria political / election news (parties, INEC, primaries). Same NEWS_API_URL + NEWS_API_KEY as crisis news.
 */
export async function fetchNigeriaPoliticalNews(limit: number = 25): Promise<NewsItem[]> {
  ...
  try {
    ...
    const payload = response.data as any;
    const articles: any[] = Array.isArray(payload?.articles)
      ? payload.articles
    ...
    return articles.slice(0, limit).map((a) => ({
      title: String(a.title ?? ""),
      content: String(a.description ?? a.content ?? a.summary ?? ""),
      timestamp: String(
        a.publishedAt ?? a.published_at ?? a.created_at ?? new Date().toISOString(),
      ),
      url: typeof a.url === "string" ? a.url : undefined,
      source:
